...
def check_spell(word)
    # T5 모델 로드
    model = T5ForConditionalGeneration.from_pretrained(j5nget5-typos-corrector)
    tokenizer = T5Tokenizer.from_pretrained(j5nget5-typos-corrector)
    device = cuda0 if torch.cuda.is_available() else cpu
    # device = mps0 if torch.cuda.is_available() else cpu # for mac m1
